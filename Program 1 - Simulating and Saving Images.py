...
"""
Credit for the base of makeGaussian function goes to - giessel 
...
def makeGaussian(size, fwhm, center=None):
    ...
    whichcan be thought of as an effective radius.
    """
    newgauss = []
...
def Plot_Shapes(num_points, shape, number): #Input number of random pixels and detect them, 
    ...
    if shape.upper() in ("SQUARE","ALL"):     
        ...
        for sq_coord in (numsquare_coord_list): #for each maximum 
            makeGaussian(random.randint(3,9),random.randint(5,9),)  #call function to make individual guassian distributions for each point
            gaussianList = newgauss
            sq_index = 0
    ...
    if not os.path.exists('E:\Stochastic Imaging Project\ImageBin\image' + str(number) + '.jpg'):
        img.save(r'E:\Stochastic Imaging Project\ImageBin\image' + str(number) + '.jpg')
    #if it does exist add 5000 to number to make it new
    else:
        img.save(r'E:\Stochastic Imaging Project\ImageBin\image' + str(number+5000) + '.jpg')
             
def Run_Multiple(howMany):
    #create directory named 'folder'
    newpath = r'E:\Stochastic Imaging Project\ImageBin'
    if not os.path.exists(newpath):
        os.makedirs(newpath)

    #run square how many times wanted
    for i in range(howMany):
        Plot_Shapes(1, "square", i) #num points per image, which images, nameof image
